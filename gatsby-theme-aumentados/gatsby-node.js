  ...
  `).then(result => {
    ...
    pages.forEach(({ node }) => {
      ...
      if (node.frontmatter.status === true) {
        createPage({
          ...
          component: path.resolve(
            rootDir,
            `gatsby-theme-aumentados/src/templates/one-column.js`
          ),
          context: {
            title: node.frontmatter.title,
            content: node.html,
    ...
    posts.forEach(({ node }) => {
      if (node.frontmatter.topology === "posts") {
        createPage({
          ...
          component: path.resolve(
            rootDir,
            "gatsby-theme-aumentados/src/templates/single-post.js"
          ),
          context: {
            slug: node.fields.slug,
            thePost: node,
    ...
    categories.forEach(category => {
      createPage({
        ...
        component: path.resolve(
          rootDir,
          "gatsby-theme-aumentados/src/templates/category-list-page.js"
        ),
        context: {
